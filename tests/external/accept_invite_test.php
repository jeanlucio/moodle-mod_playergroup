...
final class accept_invite_test extends advanced_testcase {
    ...
    /**
     * Test that accept_invite throws instead of silently succeeding when the user cannot
     ...
     * return value, accept_invite went on to award a grade and fire member_joined for a
    ...
    public function test_execute_throws_when_add_member_fails(): void {
        ...
        $this->getDataGenerator()->enrol_user($sender->id, $course->id, 'student');
        $this->setUser($sender);
        $created = create_group::execute($cm->cmid, 'Alpha', '', '🛡', 0, '');
